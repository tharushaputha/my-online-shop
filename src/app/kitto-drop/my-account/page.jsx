...
export default function MyAccountPage() {
  ...
  // --- Render Loading ---
  if (isLoading) { return ( <> <Header /> <main className="bg-white min-h-[calc(100vh-150px)] flex justify-center items-center"> <FaSpinner className="animate-spin text-pink-600 text-4xl" /> </main> <Footer /> </> ); }
  // --- Render Error ---
      ...
      <main className="bg-gray-50 min-h-[calc(100vh-150px)] py-6 sm:py-10 px-4">
         <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mb-6 sm:mb-8">
                <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-1 sm:mb-2"> Drop Shop: <span className="text-pink-600">{accountDetails.shop_name}</span> </h1>
                ...
                <div className="lg:col-span-2 space-y-5 sm:space-y-6">
                    ...
                    <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-gray-100">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-center">
                            <div> <p className="text-xl sm:text-2xl font-bold text-pink-600">Rs. {stats.totalEarnings.toFixed(2)}</p> <p className="text-xs sm:text-sm text-gray-500">Total Earnings</p> </div>
                            <div> <p className="text-xl sm:text-2xl font-bold text-pink-600">{stats.ordersToday}</p> <p className="text-xs sm:text-sm text-gray-500">Orders Today</p> </div>
                            <div> <p className="text-xl sm:text-2xl font-bold text-pink-600">{stats.ordersThisMonth}</p> <p className="text-xs sm:text-sm text-gray-500">Orders This Month</p> </div>
                        ...
                        <div className="mt-4 text-right"> <Link href="/kitto-drop/my-orders" className="text-xs sm:text-sm text-pink-600 hover:underline font-medium">View My Orders →</Link> </div>
                    </div>
                    {/* ==================================== */}
                    ...
                    {/* ===== Account Details Card (Restored) ===== */}
                     <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-gray-100">
                         ...
                         <div className="space-y-3 text-gray-600 text-xs sm:text-sm mb-4">
                             <p className="flex items-center"><FaUserCircle className="w-4 h-4 mr-2 text-pink-500 flex-shrink-0"/><strong>Name:</strong> <span className="ml-1 truncate">{accountDetails.full_name}</span></p>
                             <p className="flex items-center"><FaPhoneAlt className="w-4 h-4 mr-2 text-pink-500 flex-shrink-0"/><strong>Mobile:</strong> <span className="ml-1 truncate">{accountDetails.mobile_number}</span></p>
                             ...
                             <p className="flex items-center"><FaMapMarkerAlt className="w-4 h-4 mr-2 text-pink-500 flex-shrink-0"/><strong>City:</strong> <span className="ml-1 truncate">{accountDetails.city}</span></p>
                         ...
                         <Link href="/kitto-drop/edit-account" className="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-pink-600 text-pink-600 text-xs sm:text-sm font-medium rounded-md hover:bg-pink-50 transition-colors"> <FaUserEdit className="mr-1.5 h-4 w-4"/> Edit Details </Link>
                     </div>
                     {/* ========================================= */}
                    ...
                    {/* ===== Bank Details Card (Restored with Link) ===== */}
                     <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-gray-100">
                         ...
                         <Link
                            ...
                            className="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 border border-pink-600 text-pink-600 text-xs sm:text-sm font-medium rounded-md hover:bg-pink-50 transition-colors"
                         >
                     ...
                     {/* Back Link */}
                     <Link href="/kitto-drop" className="block mt-4 text-center text-pink-600 hover:underline text-xs sm:text-sm"> ← Back to Kitto Drop </Link>
