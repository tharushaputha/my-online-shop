...
export default function NewOrderPage() {
  ...
  const [totalAmount, setTotalAmount] = useState(400); // This is just for the 'Add Product' section display
  ...
  const fetchProductSuggestions = useCallback(
    _.debounce(async (term) => {
      ...
      try {
        // --- FIX: 'ads' වෙනුවට 'kitto_drop_products' table එකෙන් fetch කරනවා ---
        const { data, error } = await supabase
          .from('kitto_drop_products') // <-- අලුත් table එක
          .select('id, product_name, retail_price, stock_quantity, product_code') // <-- අලුත් columns
          .ilike('product_name', `%${term}%`) // Search by name
          .eq('is_active', true) // Active products විතරක්
          .gt('stock_quantity', 0) // Stock තියෙන ඒවා විතරක්
          .limit(5);
  ...
  // --- FIX: `handleProductSelect` එක 'kitto_drop_products' data වලට ගලපනවා ---
  const handleProductSelect = (product) => {
    setSelectedProduct(product);
    setProductSearchTerm(product.product_name); // 'title' නෙවෙයි
    setRetailPrice(product.retail_price); // 'price' නෙවෙයි
    setProductSuggestions([]);
  ...
  // --- Calculation Logic (No change, but now uses correct retailPrice) ---
  ...
  // --- Add/Remove Product Logic (FIXED - `product_name`, `retail_price` use, profit calc) ---
  const handleAddProduct = () => {
    ...
    const newItem = {
      product_id: selectedProduct.id, // This ID is from kitto_drop_products
      product_name: selectedProduct.product_name, // 'title' නෙවෙයි
      quantity: parseInt(quantity, 10),
      retail_price: parseFloat(retailPrice),
      sale_amount: parseFloat(saleAmount),
      profit: (parseFloat(saleAmount) - parseFloat(retailPrice)) * parseInt(quantity, 10), // Total profit for this line
  ...
  // --- Form Submission Logic (FIXED - profit calc, clear form) ---
  const handleSubmitOrder = async (e) => {
    e.preventDefault();
    setError('');
    setSuccessMessage('');

    // Validation (FIXED - using selectedCityName state)
    ...
    // Calculate final totals based on *all* items in the list
    const finalTotalAmount = orderItems.reduce((sum, item) => sum + (item.sale_amount * item.quantity) + item.delivery_fee, 0); // Corrected total amount logic
    const finalTotalProfit = orderItems.reduce((sum, item) => sum + item.profit, 0); // Profit is already calculated per line item
    ...
    try {
      ...
      const itemsToInsert = orderItems.map(item => ({
          ...item,
          order_id: newOrderId,
          profit: item.profit / item.quantity // Save profit *per item* to DB
        }));
    ...
    <>
                  ...
                  {showCityDropdown && selectedDistrictId && !isFetchingLocations && (
                    <ul className="absolute left-0 right-0 top-full mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-48 overflow-y-auto">
                      {filteredCities.length > 0 ? (
                        filteredCities.map(c => ( <li key={c.id} onClick={() => handleCitySelect(c)} className="px-3 py-2 hover:bg-pink-50 cursor-pointer text-sm text-gray-800"> {c.name_en} </li> ))
            ...
            <section className="bg-gray-50 p-4 sm:p-6 rounded-lg border border-gray-200">
              ...
              <div className="space-y-4 pt-4 border-t border-dashed">
                ...
                <div className="relative"> 
                  ...
                  {/* ****** FIX: Placeholder Changed ****** */}
                  <input type="text" id="productSearch" placeholder="Type Kitto Drop product name or code..." value={productSearchTerm} 
                         onChange={(e) => {setProductSearchTerm(e.target.value); setSelectedProduct(null); setRetailPrice(0);}} 
                         required={orderItems.length === 0} // Only required if no items are added
                         className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary shadow-sm" autoComplete="off"/> 
                  ...
                  {(isLoadingProducts || productSuggestions.length > 0) && ( 
                    <ul className="absolute left-0 right-0 top-full mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg z-10 max-h-40 overflow-y-auto"> 
                      ...
                      {!isLoadingProducts && productSuggestions.map(p => ( 
                        // ****** FIX: Show product_name and retail_price ******
                        <li key={p.id} onClick={() => handleProductSelect(p)} className="px-3 py-2 hover:bg-pink-50 cursor-pointer text-sm text-gray-800"> 
                          {p.product_name} <span className="text-xs text-gray-400">(Rs. {p.retail_price}) (Stock: {p.stock_quantity})</span> 
                        </li> 
                      ))} 
                    </ul> 
                  )} 
                ...
                {/* Sale, Profit, Total */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> {/* Changed to 2 cols for mobile friendlines */}
                  ...
                  {/* Removed TotalAmount from here, it's confusing */}
                </div>
                {/* Product Note */}
      ...
      <Footer />
       ...
       <style jsx global>{`
        ...
        .button-style { @apply rounded-lg text-white font-semibold transition-colors flex items-center justify-center; } /* Added */
        .disabled-button { @apply opacity-70 cursor-not-allowed; } /* Added */
