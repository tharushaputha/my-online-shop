...
// --- AdCard Component ---
// (Meka piliwalata, error nathuwa hadala thiyenne)
const AdCard = ({ ad, handleDelete, handleMarkAsSold }) => {
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md overflow-hidden flex flex-col justify-between transition-opacity duration-300 ${
        ad.is_sold ? 'opacity-60' : ''
      }`}
    >
      <Link
        href={`/ad/${ad.id}`}
        className="block hover:shadow-xl transition-shadow group"
      >
        {/* Image */}
        ...
        <div className="p-4">
          <h3
            className={`text-lg font-semibold text-gray-800 truncate mb-1 group-hover:text-primary ${
              ad.is_sold ? 'line-through' : ''
            }`}
          >
            {ad.title}
          </h3>
...
// (Mekath piliwalata, error nathuwa hadala thiyenne)
const MyAdsPage = () => {
  ...
  const fetchMyAds = useCallback(async (currentUserId) => {
    ...
    try {
      ...
      setError(`Could not fetch ads: ${error.message}`);
      setMyAds([]);
    } finally {
      setLoadingData(false);
    }
  ...
  const handleMarkAsSold = async (adId, newSoldStatus) => {
    console.log(
      `MyAdsPage: handleMarkAsSold called for ID: ${adId}, New Status: ${newSoldStatus}`
    );
    ...
    try {
      ...
      setMyAds((currentAds) =>
        currentAds.map((ad) =>
          ad.id === adId ? { ...ad, is_sold: newSoldStatus } : ad
        )
      );
  ...
  const handleDeleteAd = async (adId, imageUrls) => {
    ...
    if (
      !confirm('Are you sure you want to delete this ad? This cannot be undone.')
    ) {
    ...
    try {
      ...
      if (imageUrls && imageUrls.length > 0) {
        const filePaths = imageUrls
          .map((url) => {
            try {
              const urlObject = new URL(url); // <-- ERROR EKA THIBBE ME LAGAI
  ...
  if (authLoading || loadingData) {
    return <div className="text-center py-10">Loading your ads...</div>;
  }
  if (!session) {
